...
- 通过 yt-dlp 下载 YouTube 音频
- 通过 OpenAI 兼容 Whisper API 进行 ASR 语音转文字（长视频自动并行处理）
- AI 生成包含 Highlights 和视频详情的 Markdown 报告
- 超过 50 分钟的视频自动分段处理
...
1. **下载** — 通过 yt-dlp 获取音频
2. **分段** — 超过 50 分钟自动切分，所有分片统一压缩至 32k mono
3. **转录** — 并行调用 OpenAI 兼容 Whisper ASR API（最大并发 4）
